...
[windowPtr, rect]=Screen('OpenWindow',whichscreen,gray,[0 0 400 400]);
...
for trial=1:size(stimseq,1)
    ...
    elseif results(trial,2)==2
     ...
     MovieName = ['SPLW_',num2str(trial),'.gif'];  % gif文件名称
    ...
    fixonset=Screen('Flip', windowPtr);% onset
                     snapshoot= Screen(windowPtr,'GetImage');  % 获取一帧的图像          
            [A,map] = rgb2ind(snapshoot,256);
            if i==1
                imwrite(A,map,MovieName,'gif','LoopCount',Inf,'DelayTime',0.033);
            else
                 imwrite(A,map,MovieName,'gif','WriteMode','append','DelayTime',0.033);
            end
    end 
...
Screen('CloseAll');
if Exp0==1
    datafile=sprintf('DataSitting30\\%s_%s_%s_%s',subject,mfilename,datestr(now,30));
elseif Exp0==2
     datafile=sprintf('DataStanding30\\%s_%s_%s_%s',subject,mfilename,datestr(now,30));
end
save(datafile,'results','stimseq','ntrials','proangle','ST');
